...
고객 상품·특약 추천은 5,000건의 생성 더미데이터를 사용합니다. 9개 조건이 학습 행과
완전히 일치하지 않아도 유효한 입력이면 RandomForest의 `predict_proba()`로
상품과 해당 상품의 특약을 추정합니다. 학습 완료 모델은
`artifacts/customer_ml/model.joblib`에서 읽고, 데이터나 scikit-learn 버전이
