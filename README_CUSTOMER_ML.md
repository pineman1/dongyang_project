...
[Data Fork의 CSS Design Awards 소개 화면](https://www.cssdesignawards.com/sites/data-fork/34946/)에서 검은 배경, 청록·파랑 선, 얇은 구분선의 시각적 방향을 참고했습니다. 이 프로젝트용으로 작성한 SVG와 CSS를 사용하며, 상단 선 그래픽은 장식입니다.

- 고객 수·입력 조건 수·상품 수를 상단에 표시하고, 기본 정보 / 가족 구성 / 생활·건강 순서로 입력합니다.
- **학습 고객 예시 불러오기 ↗**는 현재 학습 자료의 고객 조건을 불러옵니다. 조건 변경 시 기존과 동일한 자료 일치 규칙으로 결과를 갱신합니다.
- 상품 점수는 순위 막대로 비교하고, 자료가 없는 상태도 별도로 안내합니다.
- 모바일에서는 화면이 세로로 배치됩니다. 키보드 포커스를 표시하고, 동작 줄이기 설정에서는 그래픽 애니메이션을 끕니다.

스타일은 `assets/customer.css`, 그래픽은 `assets/data-mesh.svg`, 화면 구성 보조 함수는 `customer_ui.py`에서 수정합니다. `.streamlit/config.toml`은 기본 어두운 테마를 지정합니다.

![고객 데이터 분석 화면 미리보기](docs/customer-ui-preview.png)
