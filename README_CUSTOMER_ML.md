...
특약 77.9%는 상품을 이미 안다는 조건의 평가입니다. 실제 2단계 추천 전체의 정답률은 40.7%입니다. 생성 코드가 확률로 정답을 부여한 더미데이터의 결과이므로 실제 고객 선호나 보험 가입 적합성을 의미하지 않습니다. 화면 점수는 보정되지 않은 분류 모델 출력이며 실제 가입 확률이 아닙니다.
...
print(result["product"], result["rider"])
```
...
입력 범위는 제공된 예시 데이터에 맞춘 나이 20~65세, 연소득 3,000~12,000만원, 직업위험등급 1~3, 자녀 0~3명입니다. 결측·지원하지 않는 범주·잘못된 상품/특약 조합은 오류로 표시합니다.
